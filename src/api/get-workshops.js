async function getWorkshops() {
  ...
  const url = `${import.meta.env.VITE_API_URL}/ideas`;
  ...
  if (!response.ok) {
    ...
    const data = await response.json().catch(() => {
      // If the response is not JSON then we will throw a generic error.`catch` will trigger if we try to turn `response` into JSON and fail. new is used here because Error is a built in class, so we're throwing a new Error.
